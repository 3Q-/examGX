  ...
  wx.login({
    success:(res) => {
      ...
      if (res.code) {
        //发起网络请求
        wx.request({
          url: weburl +'sinosafe/wx/getOpenIdByCode.action',
          data: {
...
//——————————————————————————————
    ...
    wx.request({
      url: weburl +'sinosafe/wx/findLinkManByUserPass.action', //仅为示例，并非真实的接口地址
      data: data,
      method:'POST',
      header: {
       ...
       if (res.data.dataStatus == "1") {
         var dataMain = res.data.dataMain;
          try {
            ...
            wx.setStorageSync('phone', dataMain.phone);
          } catch (e) {
            console.log("setStorage异常原因："+e)
          }
          console.log(dataMain.linkGroup.name);
          // 在这里判断进行相应的跳转
          if (dataMain.linkGroup.name=="督考"){
            wx.redirectTo({
              url: '../Verifier/V-step1/V-step1'
            })
          }else{
            wx.redirectTo({
              url: '../Supervisor/step1/step1'
            })
          }
  ...
  // 不知道自己的用户名与密码
  tologinusename:function(){
    wx.navigateTo({
      url: '../loginusename/loginusename'
      });
  },
      // 无法登录
})
