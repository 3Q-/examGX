  ...
  formSubmit: function (e) {
    ...
    try {
      ...
      data["examineePunishVo.linkManId"] = userid;
        ...
        data["examineePunishVo.photoPath"] = weburl + "sinosafe/wxPhoto/" + this.data.imgurl,
    console.log("wj提交前端传值：", data);
      if (userid && username) {
        wx.request({
          url: weburl +'sinosafe/wx/addExamPunishInfo.action', //仅为示例，并非真实的接口地址
          data: data,
          method: "POST",
          header: {
            "content-type": 'application/x-www-form-urlencoded'
