      ...
      try {
        ...
        if (userid) {
          ...
          data["papermoveVo.moveAddr"] = this.data.markers[0].address,
            ...
            data["papermoveVo.moveType"] = '1',
            data["papermoveVo.photoPath"] = weburl + "sinosafe/wxPhoto/" + this.data.imgurl,
            console.log("上报前端传值", data);
          //data.photo = this.data.photo;
          wx.request({
            url: weburl + 'sinosafe/wx/addPaperMoveInfo.action', //仅为示例，并非真实的接口地址
            data: data,
            method: "POST",
            header: {
